...
- **Owner alerting never emits security events (SECH-117)** — no file under `src/lib/alerting/` may import `security-events`: `app.error`/`prisma.error` feed the `error_spike` rule, so an alerting failure that emitted one would feed itself. Failures go through `alertingFailure()`. It also must **not use the shared `@/lib/prisma` client**: that client's `$on("error")` emits `prisma.error` for every failed query, so an alerting query failing would re-enter `observe()` in a loop — `store.ts` owns its own `alertingDb`, and `alerting-store.itest.ts` proves it. Off unless `ALERTING_ENABLED` is exactly `"true"`. Details: `.context-docs/alerting.md`.
