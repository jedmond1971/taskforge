...
**Direct pushes to `main` are rejected** (`GH006`). Branch protection requires the `Verify`, `Integration (cross-tenant)`, `Secret scan` and `Dependency audit` checks with admins included, so it applies to Claude Code's `jedmond1971` pushes too. Commit on a branch, then run `git push -u origin HEAD`, `gh pr create --base main --fill`, `gh pr checks --watch`, and `gh pr merge --squash --delete-branch`. When `main` has moved, the merge is refused ("head branch is not up to date") — `gh pr update-branch` does **not** exist in the installed `gh` 2.45.0, so update the branch yourself: `git merge origin/main` on the branch, push, and wait for CI to re-run. **Never lift protection (break-glass) without Jamie's explicit approval in the session.** The full recipe, the break-glass procedure, and how to add a newly required check live in `.context-docs/release-controls.md`.
...
- **Owner alerting never emits security events (SECH-117)** — no file under `src/lib/alerting/` may import `security-events`: `app.error`/`prisma.error` feed the `error_spike` rule, so an alerting failure that emitted one would feed itself. Failures go through `alertingFailure()`. Off unless `ALERTING_ENABLED` is exactly `"true"`. Details: `.context-docs/alerting.md`.
