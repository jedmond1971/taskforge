...
**`prisma migrate diff --from-url "$DATABASE_URL" --to-schema-datamodel prisma/schema.prisma` never exits 0 in this repo** (checked 2026-09-25) — it reports pre-existing drift unrelated to any new migration: the raw-SQL deferrable unique indexes on `DocPage(sectionId, position)` / `DocSection(docSpaceId, position)`, the `Issue(projectId, statusId, position)` unique + `statusId` FK, and `ProjectStatus` column types/defaults. So `--exit-code` is not a usable "no drift" check; instead confirm none of the diff lines mention your new tables (`… | grep -ci "<TableName>"` prints 0) and that the tables exist (`docker exec taskforge-db psql -U postgres -d taskforge -Atc "select count(*) from information_schema.tables where table_name in (...)"`).
