...
| Setting | Value | Why |
|---|---|---|
| Required status checks | `Verify`, `Integration (cross-tenant)`, `Secret scan`, `Dependency audit` (GitHub Actions app, id 15368) | The four jobs in `.github/workflows/ci.yml`. Job `name:` values **are** the check contexts — renaming a job silently un-requires it until protection is updated. |
...
`Dependency audit` (SECH-104) was added this way on 2026-09-23, with Jamie's approval in-session. Confirm the current list any time with:

```bash
gh api repos/jedmond1971/taskforge/branches/main/protection --jq .required_status_checks.contexts
```

Changing branch protection is a repo-admin action — Claude Code must have Jamie's approval in the session before running it, and should re-read the full protection object afterwards to confirm `enforce_admins`, `strict` and the force-push/deletion blocks are untouched (the `PATCH` above replaces only the status-check list, but a wrong call elsewhere on this endpoint can silently drop settings).
