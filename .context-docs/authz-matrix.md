...
- SECH-97 — fixed 2026-09-22: CI runs the suite (`integration` job) and coverage now extends to every admin action row by row (`admin-actions.itest.ts`), sprint actions in SPRINT mode (`sprint-actions.itest.ts`), invites + OAuth consent/token/register (`auth-flows.itest.ts`), `/api/ai/*` (`ai-routes.itest.ts`), docs file upload (`cross-tenant-routes.itest.ts`) and query-language search + autocomplete scoping (`search-scoping.itest.ts`). One defect found and fixed: `/api/oauth/token`'s refresh grant checked `revokedAt` and then revoked unconditionally, so N concurrent replays of one refresh token each minted a fresh token pair; it now claims the token with `updateMany where revokedAt: null` like the authorization-code path.
- SECH-98 — external security review before paid launch.
