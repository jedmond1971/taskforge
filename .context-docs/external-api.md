...
- **Rate limiting** (SECH-107): durable, in the Postgres `RateLimitAttempt` store. 100 req/min per `apiKeyId` (attempt-counted), and 20 missing/unknown/revoked-key failures per 15 min per IP. Survives redeploys and is shared across instances. See `rate-limiting.md`.
