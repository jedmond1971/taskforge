...
**Not yet grant-aware (Phase 2, tracked as a JFR follow-up issue):** the docs module (`resolveDocCtx`/`resolvePage`/`resolveSection` and their 5 route consumers under `src/app/api/docs/`), `/api/issues/[issueId]/route.ts`, and the MCP server tool guards (`src/lib/mcp/server.ts`, 3 call sites). These all call `canEditIssues`/`canManageProject` with just a role, no grants — a Group boost silently has no effect there yet. Wiring them requires threading an `orgId` + grants fetch through helper functions that don't currently return one.
