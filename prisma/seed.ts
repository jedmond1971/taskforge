...
async function main() {
  ...
  // Default status data for every project
  const defaultStatusData = [
    { name: "To Do",       category: "TODO"        as const, position: 0, isDefault: true  },
    { name: "In Progress", category: "IN_PROGRESS" as const, position: 0, isDefault: true  },
    { name: "In Review",   category: "IN_PROGRESS" as const, position: 1, isDefault: false },
    { name: "Done",        category: "DONE"        as const, position: 0, isDefault: true  },
    { name: "Cancelled",   category: "DONE"        as const, position: 1, isDefault: false },
  ];

  // Helper: returns a statusId lookup function for a project
  async function loadStatuses(projectId: string) {
    const statuses = await prisma.projectStatus.findMany({ where: { projectId } });
    return (name: string) => {
      const s = statuses.find((x) => x.name === name);
      if (!s) throw new Error(`Status "${name}" not found for project ${projectId}`);
      return s.id;
    };
  }
  ...
  const pl = await prisma.project.create({
    data: {
      ...
      orgId: aliceOrg.id,
      statuses: { createMany: { data: defaultStatusData } },
    },
  });
  ...
  // Load status ID helpers for each project
  const plSid = await loadStatuses(pl.id);
  const maSid = await loadStatuses(ma.id);
  const wrSid = await loadStatuses(wr.id);
