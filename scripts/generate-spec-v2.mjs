...
const sections = [
  ...
  bullet('Docs module Phase 7 QA — 54 Vitest tests added (src/__tests__/docs.test.ts); resolveDocCtx, all route handlers, permission matrix, revision side-effect, search snippet logic'),
  bullet('Docs module search, public docspace visibility, sidebar navigation, and permission enforcement documented (Phases 5–6)'),
  ...
  bullet('JFR-107 housekeeping pass: Archive→isClosed terminology corrected, IssueStatus enum replaced with ProjectStatus/StatusCategory throughout, session invalidation (sessionVersion) documented, Custom Fields documented, dead Board/Column models removed from the schema and this spec'),
  ...
  bullet('User.sessionVersion (Int, default 1) is a monotonically increasing counter checked on every auth() call.'),
  bullet('The JWT callback fetches the current DB value each request and sets token.invalidated = true if it no longer matches the token — getCurrentUser() then returns null, so requireAuth/requireAdmin and page-layout guards treat the session as logged out on its next request.'),
  bullet('Three events bump the counter: adminResetUserPassword, adminUpdateUser (role changes only), and self-service changePassword.'),
  bullet('After a self-service password change, the client calls session.update(), which re-arms the active browser’s token by syncing its version forward — keeping that session logged in while all other sessions for the user are invalidated.'),
  note('Invalidation is enforced at the server-component/server-action layer, not at Edge middleware — Prisma cannot run in the Edge runtime. An invalidated session still passes the middleware’s auth check but is caught by getCurrentUser() on the first subsequent server call.'),
  ...
  bullet('Status — FK to a ProjectStatus row (per-project, admin-configurable name/position; each status carries a StatusCategory of TODO | IN_PROGRESS | DONE for board-column grouping)'),
  ...
  h3('Custom Fields'),
  para('Organizations can define custom fields (CustomField model) scoped at the org level, then restrict which projects expose each field via CustomFieldProject.'),
  bullet('Field types (CustomFieldType): text, number, date, boolean, single-select, and multi-select.'),
  bullet('CustomFieldValue stores one typed value per (customField, issue) pair — textValue, numberValue, dateValue, boolValue, selectValue, or multiValues[] depending on the field type.'),
  bullet('Custom fields render on the issue detail view alongside built-in fields once a project opts in via CustomFieldProject.'),

  // 4.3 Kanban Board
  h2('4.3  Kanban Board'),
  bullet('Issues are displayed as cards within status columns (one column per ProjectStatus row, ordered by category then position).'),
  bullet('Cards can be dragged between columns to update issue status in real time.'),
  bullet('Statuses themselves are admin-configurable per project (BoardSettings.tsx): add, rename, delete, and reorder ProjectStatus rows within a StatusCategory.'),
  ...
  bullet('isPublic controls public visibility. PROJECT_LEAD can toggle via PATCH /api/docs/[projectKey] with { isPublic: boolean }.'),
  bullet('When isPublic is true, any authenticated JedForge user can read all pages in the docspace, even without a ProjectMember row. Write operations (create, edit, delete) still require an active project membership.'),
  bullet('resolveDocCtx (src/app/api/docs/_helpers.ts) handles all access resolution: it upserts the docspace for members, returns role=null for public non-members, and returns null (deny) for non-members on private docspaces.'),
  h3('Sections'),
  ...
  h3('Docs Search'),
  bullet('GET /api/docs/[projectKey]/search?q=<query> — case-insensitive substring search across page titles and NATIVE page content.'),
  bullet('Returns up to 20 results ordered by updatedAt desc, each with: id, title, type, sectionId, sectionTitle, snippet, updatedAt.'),
  bullet('snippet is a plain-text excerpt (HTML tags stripped) around the matching term for NATIVE pages; null for DOCUMENT pages.'),
  bullet('Accessible to any user with read access to the docspace (project members and public non-members).'),
  h3('Sidebar Navigation'),
  bullet('The project docs layout (src/app/(dashboard)/projects/[projectKey]/docs/layout.tsx) renders a persistent left sidebar.'),
  bullet('The sidebar lists all sections with their child pages, plus any top-level pages (sectionId: null).'),
  bullet('DocSpace data is fetched server-side in the layout, which also upserts the DocSpace if it does not yet exist.'),
  bullet('Active page is highlighted; navigating to a page does not trigger a full-page reload.'),
  h3('Docs Permission Enforcement'),
  bullet('Read (GET any docs route): any project member, or any authenticated user on a public docspace (role=null).'),
  bullet('Create pages and sections, file upload: TEAM_MEMBER or PROJECT_LEAD (canEditIssues).'),
  bullet('Edit pages and sections (PATCH): TEAM_MEMBER or PROJECT_LEAD (canEditIssues).'),
  bullet('Delete pages and sections (DELETE): PROJECT_LEAD only (canManageProject).'),
  bullet('Toggle docspace visibility (PATCH /api/docs/[projectKey]): PROJECT_LEAD only — checked inline, not via resolveDocCtx.'),
  h3('Docs Module Test Coverage'),
  bullet('src/__tests__/docs.test.ts — 54 Vitest unit tests (Phase 7 / JFDOCS-7).'),
  bullet('Coverage: resolveDocCtx (5 cases), docspace GET/PATCH, pages GET/POST/PATCH/DELETE, sections GET/POST/PATCH/DELETE, search including HTML stripping, revisions GET.'),
  bullet('Permission matrix verified for every write endpoint across PROJECT_LEAD, TEAM_MEMBER, VIEWER, and public non-member (role=null).'),
  ...
  bullet('Projects can be closed by an Admin (isClosed) — the only deactivation mechanism; there is no separate Archive concept. Closed projects drop off the main Projects page, Dashboard, and closed-project URLs redirect non-admins, except the Docs module which remains accessible.'),
  ...
  headerTable(
    ...
    [
      ...
      ['Project',       'id, name, key (unique), orgId, isPrivate, isClosed', 'Belongs to one Organization. isClosed: admin-only close/reopen — there is no Archive concept.'],
      ['ProjectMember', 'id, userId, projectId, role (ProjectMemberRole)', 'Role: PROJECT_LEAD | TEAM_MEMBER | VIEWER. Unique (userId, projectId).'],
      ['Issue',         'id, key (unique), title, description, statusId, priority, type, assigneeId, reporterId, parentId, position, labels[], dueDate', 'statusId: FK to ProjectStatus. priority: IssuePriority enum. type: IssueType enum.'],
      ['ProjectStatus', 'id, projectId, name, category (StatusCategory), position, isDefault', 'Per-project, admin-configurable Kanban column. Replaces the old fixed IssueStatus enum. Unique (projectId, name).'],
      ...
      ['CustomField',   'id, orgId, name, type (CustomFieldType), options[], position', 'Org-scoped field definition. Unique (orgId, name).'],
      ['CustomFieldProject', 'id, customFieldId, projectId', 'Junction table — restricts which projects expose a CustomField. Unique (customFieldId, projectId).'],
      ['CustomFieldValue',   'id, customFieldId, issueId, textValue, numberValue, dateValue, boolValue, selectValue, multiValues[]', 'One typed value per (customField, issue). Unique (customFieldId, issueId).'],
  ...
  headerTable(
    ...
    [
      ...
      ['StatusCategory',    'TODO | IN_PROGRESS | DONE  (groups ProjectStatus rows for board-column layout)'],
      ...
      ['CustomFieldType',   'text | number | date | boolean | single-select | multi-select'],
      ['Plan',              'FREE | PRO | TEAM'],
  ...
  headerTable(
    ...
    [
      ...
      ['Close / reopen projects (isClosed)',  'No',  'No',  'Yes'],
  ...
  headerTable(
    ...
    [
      ...
      ['Create / edit doc pages and sections', 'No',  'Yes', 'Yes'],
      ['Delete doc pages and sections',        'No',  'No',  'Yes'],
      ['Toggle docspace visibility (isPublic)','No',  'No',  'Yes'],
  ...
  bullet('IssueStatus is not an enum — statusId accepts a ProjectStatus cuid, a human name ("Done"), or a category key ("DONE"). IssuePriority is a true enum synthesised in API responses (not a DB table).'),
