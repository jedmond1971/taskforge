...
function formatAction(entry: ActivityEntry): React.ReactNode {
  if (entry.action === "created") {
    return <span className="text-zinc-400">created this issue</span>;
  }
  if (entry.action === "commented") {
    return <span className="text-zinc-400">added a comment</span>;
  }
  if (entry.action === "updated" && entry.field) {
    ...
    return (
      <span className="text-zinc-400">
        changed <span className="text-zinc-300 font-medium">{fieldLabel}</span>{" "}
        from{" "}
  ...
  return <span className="text-zinc-400">{entry.action}</span>;
}
            ...
            <div className="flex-1 min-w-0 text-sm">
              <p className="leading-snug">
                ...
                {showIssue && entry.issue && (
                  <>
                    {" "}on{" "}
                    <span className="font-mono text-xs text-indigo-400">{entry.issue.key}</span>{" "}
