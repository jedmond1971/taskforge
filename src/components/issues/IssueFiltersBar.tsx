...
export function IssueFiltersBar({ members, currentFilters }: IssueFiltersBarProps) {
  ...
  const selectClass = "px-3 py-2 sm:py-1.5 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-zinc-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer min-h-[44px] sm:min-h-0 w-full sm:w-auto";
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:flex-wrap">
      ...
      {hasFilters && (
        <button
          onClick={clearAll}
          className="flex items-center justify-center gap-1 px-3 py-2 sm:py-1.5 text-sm text-zinc-400 hover:text-zinc-200 border border-zinc-700 rounded-lg hover:bg-zinc-800 transition-colors min-h-[44px] sm:min-h-0"
        >
